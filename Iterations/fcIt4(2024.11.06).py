    ...
    if cost_info["final_cost"] != cost_info["cost_withclass"]:
        print(
            "Normally this flight would come to ${}, but today you'd get {}% off!".format(
                ...
                float(cost_info["final_cost"]) / float(cost_info["cost_withclass"]) * 100 - 100,
            )
        )
...
def collect_client_data():
    ...
    while dest_is_int == False or int(temp_dest) > 4 or int(temp_dest) < 1:
        temp_dest = input("Please input a number from 1-4\n")
        try:
            info["destination"] = destination_list[int(temp_dest)-1]
        except:
            dest_is_int = False
